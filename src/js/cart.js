...
(() => {
    ...
    const parsePrice = (txt) =>
      Number(String(txt || '0').replace(/[^\d,.-]/g, '').replace(/\./g, '').replace(',', '.')) || 0;
    ...
    function readCardData(card) {
      ...
      const name = card.dataset.name || card.querySelector('h3')?.innerText?.trim() || 'Produto';
      const priceText = card.dataset.price || card.querySelector('.product-price')?.innerText || 'R$ 0,00';
    ...
    function renderCart() {
      ...
      list.innerHTML = cart.map((it, i) => `
        <div class="cart-line" data-i="${i}" style="display:grid;grid-template-columns:48px 1fr auto;gap:10px;align-items:center;margin:8px 0;">
          <img src="${it.img || '../imgs/bolsa.png'}" alt="${it.name}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;background:#eee">
          <div>
            <div style="font-weight:600">${it.name}</div>
            <div style="font-size:.9rem;color:#555">${BRL.format(it.price)}</div>
            <div class="qty" style="margin-top:6px;display:flex;gap:6px;align-items:center">
              ...
              <input class="inp-qty" type="number" min="1" value="${it.qty}" style="width:60px;padding:4px 6px">
      ...
      totalEl.textContent = BRL.format(totalCart(cart)).replace('R$ ', '').replace('R$', '');
    }
  
    // --------- Abrir/fechar sidebar ---------
    function openCart()  { const s = $('#cartSidebar'); if (s) s.style.display = 'block'; }
    function closeCart() { const s = $('#cartSidebar'); if (s) s.style.display = 'none'; }
    function toggleCart() {
      const s = $('#cartSidebar'); if (!s) return;
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.body.addEventListener('click', (e) => {
        ...
        if (!p.price || !p.name) return console.warn('Card sem dados suficientes', card);
        addToCart(p);
      });
